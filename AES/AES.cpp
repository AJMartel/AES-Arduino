...
void AES::set_IV_array(byte *ivc1){
  memcpy(iv,ivc1,16);
}
...
void AES::padPlaintext(byte* in,byte* out)
{
	//memcpy(out,in,size);
	if(pad==N_BLOCK){
	for (int i = 0; i < N_BLOCK; i++){
		out[i] = arr_pad[N_BLOCK - 1];
	}
	}else{
...
void AES::unpadPlaintext(byte* in) {
    int unpadlen = size - (int)in[size - 1];
    for (int i = unpadlen; i < size; i++){;
		in[i] = 0x00;
	}
    size = unpadlen;
}
...
void AES::do_aes_encrypt(byte *plain,int size_p,byte *cipher){
	calc_size_n_pad(size_p);
	byte plain_p[N_BLOCK];
	padPlaintext(plain,plain_p);
	int blocks = get_size() / N_BLOCK;

	//set_key (key, bits) ;
	if(pad==N_BLOCK)blocks--;
    int seg = 0;
	while(blocks&&cbc_encrypt(plain+(seg*N_BLOCK), cipher+(seg*N_BLOCK), 1)==SUCCESS){
	    seg++;
        if(seg>=blocks)
            break;
	}
	...
	if(pad==N_BLOCK){
        xor_block (iv, plain_p) ;
        encrypt (iv, iv);
        copy_n_bytes (cipher+size-N_BLOCK, iv, N_BLOCK) ;
	}
}
...
void AES::do_aes_decrypt(byte *cipher,int size_c,byte *plain,byte *key, int bits, byte ivl [N_BLOCK]){
	...
	unpadPlaintext(plain);
}

/******************************************************************************/

void AES::do_aes_decrypt(byte *cipher,int size_c,byte *plain){
	set_size(size_c);
	int blocks = size_c / N_BLOCK;
	//set_key (key, bits);
